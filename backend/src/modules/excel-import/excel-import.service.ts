    ...
    try {
      ...
      firstRowKeys.forEach((key) => {
        ...
        switch (cleanKey) {
          case 'name':
            ...
            break;
          case 'phonenumber':
            keyMap[key] = 'phoneNumber';
            break;
          case 'salarygross':
            keyMap[key] = 'salaryGross';
            break;
          case 'idnumber':
            keyMap[key] = 'idNumber';
            break;
          case 'krapin':
            ...
            break;
          case 'nssfnumber':
            keyMap[key] = 'nssfNumber';
            break;
          case 'nhifnumber':
            keyMap[key] = 'nhifNumber';
            break;
          case 'jobtitle':
            keyMap[key] = 'jobTitle';
            break;
          case 'startdate':
            keyMap[key] = 'startDate';
            break;
          case 'dateofbirth':
            keyMap[key] = 'dateOfBirth';
            break;
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          // Create worker
          const worker = await this.workersService.create(userId, {
            name: row.name!.trim(),
    ...
    const instructions = [
      {
        'Employee Import Instructions':
          'Please follow these guidelines when filling in employee data:',
      },
      { 'Employee Import Instructions': '' },
      { 'Employee Import Instructions': '🔴 REQUIRED FIELDS (marked with *)' },
      {
        'Employee Import Instructions':
          '  - name*: Full employee name (e.g., "John Doe")',
      },
      {
        'Employee Import Instructions':
          '  - phoneNumber*: Phone with country code (e.g., "+254700000000")',
      },
      {
        'Employee Import Instructions':
          '  - salaryGross*: Monthly gross salary as a number (e.g., "25000")',
      },
      { 'Employee Import Instructions': '' },
      ...
      {
        'Employee Import Instructions':
          '  - kraPin: KRA PIN (format: A001234567X)',
      },
      {
        'Employee Import Instructions':
          '  - nssfNumber: NSSF registration number',
      },
      {
        'Employee Import Instructions':
      ...
      {
        'Employee Import Instructions':
          '  - startDate: Employment start date (YYYY-MM-DD format)',
      },
      {
        'Employee Import Instructions':
