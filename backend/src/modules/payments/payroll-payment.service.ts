      ...
      try {
        while (remainingAmount > 0) {
          ...
          const transaction = this.transactionRepository.create({
            ...
            accountReference: phone.replace(/[^\d]/g, ''),
            metadata: {
              payrollRecordId: record.id,
              workerName: record.worker.name,
              provider: 'MPESA',
              recipientAccount: phone.replace(/[^\d]/g, ''),
  ...
  private async processBankBatch(records: PayrollRecord[]) {
    ...
    for (const record of records) {
      ...
      const transaction = this.transactionRepository.create({
        ...
        accountReference: record.worker.bankAccount,
        metadata: {
          payrollRecordId: record.id,
          workerName: record.worker.name,
          provider: 'PESALINK',
          ...
          bankCode: record.worker.bankCode,
          recipientAccount: record.worker.bankAccount,
          taxBreakdown: record.taxBreakdown,
          grossSalary: record.grossSalary,
          netPay: record.netSalary,
          estimatedFee: fee,
