...
export class BalanceSyncTask {
    ...
    async syncBalances() {
        ...
        for (const user of users) {
            try {
                const walletInfo = await this.intaSendService.getWalletBalance(user.intasendWalletId);
                ...
                if (diff > 10) { // Tolerate small differences, log significant drift
                    this.logger.warn(
                ...
                this.logger.error(`Failed to sync balance for user ${user.email}`, error);
            }
        }
