...
describe('PayrollService', () => {
    ...
    let taxesService: { calculateTaxes: jest.Mock; generateTaxSubmission: jest.Mock };
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                },
                {
                    ...
                    useValue: { generatePayslipsBatch: jest.fn(), generatePayslip: jest.fn() },
                },
                {
                    provide: DataSource,
                    useValue: {
                        transaction: jest.fn((cb) => cb({ save: jest.fn(), findOne: jest.fn() })),
                    },
                },
            ],
        }).compile();
    ...
    describe('calculatePayrollForUser', () => {
        it('should calculate payroll for valid workers correctly', async () => {
            ...
            // Assertions
            expect(workersRepo.find).toHaveBeenCalledWith({ where: { userId: 'user-1', isActive: true } });
        ...
        it('should handle tax calculation errors gracefully', async () => {
            workersRepo.find.mockResolvedValue([mockWorker]);
            taxesService.calculateTaxes.mockRejectedValue(new Error('Tax service down'));
