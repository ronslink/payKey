...
import { PayPeriod, PayPeriodStatus, PayPeriodFrequency } from './entities/pay-period.entity';
...
describe('PayPeriodsService', () => {
    ...
    let payrollRecordRepo: { find: jest.Mock; count: jest.Mock; update: jest.Mock };
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(PayrollRecord), useValue: payrollRecordRepo },
                { provide: getRepositoryToken(TaxSubmission), useValue: { findOne: jest.fn(), create: jest.fn(), save: jest.fn() } },
                { provide: TaxPaymentsService, useValue: { ensureObligation: jest.fn() } },
            ],
        }).compile();
    ...
    describe('create', () => {
        ...
        it('should reject if start date is after end date', async () => {
            ...
            await expect(service.create(dto as any, 'user-1')).rejects.toThrow('Start date must be before end date');
        });
        ...
        it('should reject overlapping pay periods', async () => {
            ...
            await expect(service.create(dto as any, 'user-1')).rejects.toThrow('Pay period overlaps with existing period');
        });
    });
    ...
    describe('validateStatusTransition (via update)', () => {
        it('should allow DRAFT -> ACTIVE', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.DRAFT });

            await expect(service.update('period-1', { status: PayPeriodStatus.ACTIVE })).resolves.toBeDefined();
        });

        it('should allow PROCESSING -> COMPLETED', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.PROCESSING });

            await expect(service.update('period-1', { status: PayPeriodStatus.COMPLETED })).resolves.toBeDefined();
        });

        it('should reject invalid transitions (DRAFT -> COMPLETED)', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.DRAFT });

            await expect(service.update('period-1', { status: PayPeriodStatus.COMPLETED })).rejects.toThrow(
                'Invalid status transition from DRAFT to COMPLETED',
            );
        });

        it('should reject any transition from CLOSED', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.CLOSED });

            await expect(service.update('period-1', { status: PayPeriodStatus.ACTIVE })).rejects.toThrow(BadRequestException);
        });
    });

    describe('remove', () => {
        it('should prevent deletion of COMPLETED periods', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.COMPLETED });
    ...
    describe('process', () => {
        it('should require payroll records to process', async () => {
            payPeriodRepo.findOne.mockResolvedValue({ ...mockPayPeriod, status: PayPeriodStatus.ACTIVE });
        ...
        it('should calculate totals and update status when processing', async () => {
            ...
            expect(payPeriodRepo.update).toHaveBeenCalledWith('period-1', expect.objectContaining({
