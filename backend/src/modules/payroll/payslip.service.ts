...
export class PayslipService {
  ...
  private readonly pdfCache = new Map<string, { buffer: Buffer; timestamp: number }>();
  ...
  async generatePayslip(record: PayrollRecord, useCache = true): Promise<Buffer> {
    ...
    // Check cache
    if (useCache && this.pdfCache.has(cacheKey)) {
      const cached = this.pdfCache.get(cacheKey)!;
      if (Date.now() - cached.timestamp < this.CACHE_TTL) {
        this.logger.debug(`Cache hit for payslip ${record.id}`);
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      if (useCache) {
        this.pdfCache.set(cacheKey, { buffer, timestamp: Date.now() });
        this.cleanupCache();
      }
      return buffer;
    });
  ...
  private cleanupCache(): void {
    ...
    keysToDelete.forEach((key) => this.pdfCache.delete(key));
  ...
  clearCache(): void {
    this.pdfCache.clear();
    this.logger.log('Payslip cache cleared');
  }
