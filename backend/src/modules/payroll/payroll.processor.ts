...
export class PayrollProcessor {
    ...
    async handlePayrollProcessing(job: Job<{ userId: string; payPeriodId: string; skipPayout: boolean }>) {
        this.logger.log(`Processing payroll job ${job.id} for period ${job.data.payPeriodId}`);
