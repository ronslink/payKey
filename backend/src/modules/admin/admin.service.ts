...
export class AdminService {
  ...
  private async getDbHealth() {
    try {
      ...
      return {
        ...
        longestQuerySeconds: parseFloat(
          connResult[0]?.longest_query_seconds || '0',
        ),
  ...
  async getUserDetail(userId: string) {
    const [user, workers, recentTransactions, subscription, payPeriods] =
      await Promise.all([
  ...
  async getPayPeriods(search?: string, page = 1, limit = 20) {
    ...
    const placeholders = employerIds
  ...
  async logAction(data: {
    ...
    try {
      ...
      this.logger.error(
        `Failed to create audit log for ${data.action} on ${data.entityType}`,
        error,
      );
    }
  }
  ...
  async getAuditLogs(filters: {
    ...
    if (filters.adminEmail) {
      query.andWhere('u.email ILIKE :adminEmail', {
