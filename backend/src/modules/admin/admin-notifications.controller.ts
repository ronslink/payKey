  ...
  async sendNotification(
    ...
    if (!body.broadcast && !body.maintenance && !hasTiers && !hasUsers) {
      throw new BadRequestException(
        'Must provide tiers, userIds, maintenance, or set broadcast to true',
      );
    }
    ...
    if (body.broadcast) {
      ...
      if (activeUserIds.length > 0) {
        users = await this.userRepo.find({
          where: {
            id: In(activeUserIds),
            role: In([UserRole.EMPLOYER, UserRole.USER]),
          },
        });
      }
    } else if (hasTiers) {
      ...
      users = await this.userRepo
        ...
        .where('u.role IN (:...roles)', {
          roles: [UserRole.EMPLOYER, UserRole.USER],
        })
    ...
    const deviceTokenMap: Map<string, string[]> = new Map();
    ...
    if (body.maintenance) {
      ...
      if (body.maintenanceWindow?.startsAt)
        notifMeta.maintenanceStartsAt = body.maintenanceWindow.startsAt;
      if (body.maintenanceWindow?.endsAt)
        notifMeta.maintenanceEndsAt = body.maintenanceWindow.endsAt;
    }
    ...
    this.adminService.logAction({
      ...
      newValues: {
        ...
        maintenanceWindow: body.maintenanceWindow ?? null,
      },
      ipAddress: req.ip,
    });
