...
export class CampaignScheduler {
  ...
  async handleCampaignDispatch(): Promise<void> {
    ...
    this.logger.log(
      `[CampaignScheduler] dispatching ${due.length} campaign(s)`,
    );
  ...
  private async dispatchEmail(
    ...
    results
      ...
      .forEach((r) =>
        this.logger.error(
          '[CampaignScheduler] email promise rejected:',
          r.reason,
        ),
      );
  ...
  private async dispatchPush(
    ...
    const pushData: Record<string, string> = {
      ...
      ...(campaign.promotionalItemId
        ? { promotionalItemId: campaign.promotionalItemId }
        : {}),
    };
      ...
      tokens.map(async (deviceToken) => {
        ...
        if (result.success) {
          ...
          if (
            ...
            this.logger.log(
              `[CampaignScheduler] deactivated stale token ${deviceToken.token}`,
            );
          }
        }
      }),
    ...
    results
      ...
      .forEach((r) =>
        this.logger.error(
          '[CampaignScheduler] push promise rejected:',
          r.reason,
        ),
      );
