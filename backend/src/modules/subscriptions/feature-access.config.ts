...
export const FEATURE_ACCESS_MATRIX: FeatureDefinition[] = [
    ...
    // ============== PLATINUM TIER FEATURES ==============
    {
        ...
        description: 'Track vacation, sick days, and time off with approval workflow',
        tiers: ['PLATINUM'],
        mockDataAvailable: true,
        displayOrder: 30,
    },
 ...
 */
export function tierHasFeature(tier: SubscriptionTier, featureKey: string): boolean {
 ...
 */
export function getLowestTierForFeature(featureKey: string): SubscriptionTier | null {
    const feature = getFeatureByKey(featureKey);
...
export function getUpgradeFeatures(
    ...
    const currentFeatures = new Set(getFeaturesForTier(currentTier).map((f) => f.key));
    return getFeaturesForTier(targetTier).filter((f) => !currentFeatures.has(f.key));
}
