...
import { LeaveRequest, LeaveStatus } from '../workers/entities/leave-request.entity';
...
export class TasksService {
    ...
    async getTasks(userId: string): Promise<TaskDto[]> {
        ...
        for (const leave of pendingLeaves) {
            const daysUntilStart = this.getDaysDifference(new Date(), leave.startDate);
            let priority = TaskPriority.LOW;
    ...
    private getDaysDifference(date1: Date, date2: Date): number {
        ...
        return Math.ceil((new Date(date2).getTime() - new Date(date1).getTime()) / oneDay);
    }
}
