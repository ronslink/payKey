...
export class UpdateSubscriptionSchema1769673652350 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "exchange_rates" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "sourceCurrency" character varying NOT NULL, "targetCurrency" character varying NOT NULL, "rate" numeric(10,4) NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_33a614bad9e61956079d817ebe2" PRIMARY KEY ("id"))`);

        // Check and Create Enums
        const hasNotifType = await queryRunner.query(`SELECT 1 FROM pg_type WHERE typname = 'notifications_type_enum'`);
        if (hasNotifType.length === 0) {
            await queryRunner.query(`CREATE TYPE "public"."notifications_type_enum" AS ENUM('SMS', 'EMAIL', 'PUSH')`);
        }

        const hasNotifStatus = await queryRunner.query(`SELECT 1 FROM pg_type WHERE typname = 'notifications_status_enum'`);
        ...
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_692a909ee0fa9383e7859f9b40" ON "notifications" ("userId") `);

        const hasDevicePlatform = await queryRunner.query(`SELECT 1 FROM pg_type WHERE typname = 'device_tokens_platform_enum'`);
        ...
        const hasFKNotif = await queryRunner.query(`SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'FK_692a909ee0fa9383e7859f9b406'`);
        if (hasFKNotif.length === 0) {
            await queryRunner.query(`ALTER TABLE "notifications" ADD CONSTRAINT "FK_692a909ee0fa9383e7859f9b406" FOREIGN KEY ("userId") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        }

        const hasFKDevice = await queryRunner.query(`SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'FK_511957e3e8443429dc3fb00120c'`);
