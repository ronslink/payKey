...
export class AddPerformanceIndexes1734100000000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_workers_userId_isActive"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_workers_userId_createdAt"`,
    );

    // Remove PayrollRecord indexes
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_payroll_records_payPeriodId"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_payroll_records_userId_periodStart"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_payroll_records_workerId"`,
    );

    // Remove LeaveRequest indexes
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_leave_requests_workerId_status"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_leave_requests_requestedById"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_leave_requests_workerId_startDate"`,
    );
  }
}
