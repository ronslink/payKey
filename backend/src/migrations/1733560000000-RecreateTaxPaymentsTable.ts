        ...
        await queryRunner.createTable(
            new Table({
                ...
                columns: [
                    ...
                    { name: 'taxType', type: 'enum', enum: ['PAYE', 'SHIF', 'NSSF_TIER1', 'NSSF_TIER2', 'HOUSING_LEVY'] },
                    { name: 'paymentYear', type: 'int' },
                    { name: 'paymentMonth', type: 'int' },
                    { name: 'amount', type: 'decimal', precision: 12, scale: 2 },
                    { name: 'paymentDate', type: 'date', isNullable: true },
                    { name: 'paymentMethod', type: 'enum', enum: ['MPESA', 'BANK'], isNullable: true },
                    { name: 'receiptNumber', type: 'varchar', isNullable: true },
                    { name: 'status', type: 'enum', enum: ['PENDING', 'PAID', 'OVERDUE'], default: "'PENDING'" },
