...
import { TaxType, RateType } from '../src/modules/tax-config/entities/tax-config.entity';
...
async function seed2024TaxConfigs() {
    ...
    try {
        ...
        const nssfTier2Config = {
            ...
            paymentDeadline: '9th of following month',
            notes: 'NSSF Tier II 2024: 6% of KES 7,001-36,000 (max KES 1,740 each party)',
        };
        ...
        console.error('❌ Error seeding 2024 tax configs:', error.message);
    } finally {
        await app.close();
    }
}
