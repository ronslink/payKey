...
async function bootstrap() {
    ...
    const workersRepository = app.get<Repository<Worker>>(getRepositoryToken(Worker));
    ...
    if (user) {
        ...
        if (user.linkedWorkerId) {
            const worker = await workersRepository.findOne({ where: { id: user.linkedWorkerId } });
            if (worker) {
                console.log(`Found linked WORKER. Current Phone: '${worker.phoneNumber}'`);
                ...
                console.log('Linked worker ID found on user, but worker record not found.');
            }
        } else {
        ...
        if (worker) {
            console.log(`Found WORKER (no user account yet?). Current Phone: '${worker.phoneNumber}'`);
