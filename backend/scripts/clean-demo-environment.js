...
async function cleanAndSeedDemo() {
  try {
    ...
    console.log('🧹 Cleaning and reseeding demo environment...\n');
    ...
    console.log('   ✅ Pay periods cleared');
    
    // 2. Create fresh pay periods
    console.log('\n📅 Creating fresh pay periods...');
    ...
    console.log('\n👥 Fetching workers and pay periods...');
    ...
    for (const period of payPeriodsResult.rows) {
      ...
      for (const worker of workersResult.rows) {
        ...
        payrollCount++;
        console.log(`   ✅ ${worker.name}: KES ${netSalary}`);
      }
    }
    ...
    console.log(`   • Pay Periods: 3 (Jan, Feb 2025 - COMPLETED | Mar 2025 - PROCESSING)`);
    console.log(`   • Workers: ${workersResult.rows.length} employees`);
    console.log(`   • Payroll Records: ${payrollCount} records`);
    console.log(`   • Transactions: ${transactionCount} transactions`);
    console.log(`   • Monthly Payroll: KES 456,000 gross | KES 364,800 net | KES 91,200 tax`);
    console.log(`   • Annual Payroll: KES 5,472,000 gross | KES 4,377,600 net | KES 1,094,400 tax`);
    console.log('\n🚀 The demo environment is now ready for testing!');
