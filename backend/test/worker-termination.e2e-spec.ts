...
describe('Worker Termination E2E', () => {
    ...
    it('8. Should verify payroll record and tax submission created', async () => {
        ...
        const record = payrollRes.body.find((p: any) => p.workerId === workerId && p.status === 'finalized');
        ...
        const submission = taxRes.body.find((s: any) => s.payPeriod.id === record.payPeriodId);
