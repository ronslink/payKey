    ...
    <section ref={sectionRef} className="relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-24 lg:pt-32 pb-8">
        <div className="flex flex-col items-center text-center">
          {/* Badge */}
          <div
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-medium mb-8 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
          ...
          {/* Headline */}
          <h1
            className={`text-4xl sm:text-5xl lg:text-7xl font-bold tracking-tight text-white mb-6 leading-[1.08] transition-all duration-700 delay-100 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            Pay your help, <span className="text-gradient italic">simply</span>.
            <br />
            Right from your <span className="text-gradient italic">phone</span>.
          </h1>

          {/* Subtitle */}
          <p
            className={`text-lg sm:text-xl text-slate-400 max-w-2xl mb-10 leading-relaxed transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          >
            Manage your house help, driver, nanny, or gardener like a pro.
            M-Pesa payments, clear payslips, and current PAYE, NSSF, SHIF and
            Housing Levy calculations — all from one app.
          </p>
          ...
          {/* Trust badges */}
          <div
            className={`flex items-center gap-6 mb-16 transition-all duration-700 [transition-delay:400ms] ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
            {[
              { icon: "💰", label: "M-Pesa native" },
              { icon: "📄", label: "PAYE · NSSF · SHIF" },
          ...
          {/* 3D App Screenshots */}
          <div
            className={`relative w-full max-w-5xl transition-all duration-1000 delay-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
          >
            {/* Ambient glow */}
      ...
      <div className="border-t border-white/5 bg-white/[0.02] mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-xs text-slate-500 tracking-widest uppercase">
            Built for Kenya · Effective-dated deductions · M-Pesa enabled
          </span>
          <div className="flex items-center gap-3">
            {["PAYE records", "SHIF", "NSSF", "M-Pesa"].map((tag) => (
              <span
                key={tag}
